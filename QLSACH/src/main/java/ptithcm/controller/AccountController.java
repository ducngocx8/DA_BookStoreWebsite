...
//		    int pass = 100000 + rd.nextInt(999999);
	...
	public String updatepasswordsubmit(@RequestParam("oldpassword") String oldpassword, @RequestParam("newpassword") String newpassword, @RequestParam("renewpassword") String renewpassword, RedirectAttributes ra) {
	...
	public String checklogin(HttpSession session, HttpServletRequest request, @RequestParam("username") String username, @RequestParam("password") String password, HttpServletResponse response, ModelMap model) {
		System.out.println("URL LOGIN = "+request.getRequestURI());
		if(accountServiceImpl.getInfoLogin(username, password) != null) {
			...
			if(session != null) {
				if(session.getAttribute("url_forward") != null && session.getAttribute("role") != null && (int) session.getAttribute("role") == 1) {
