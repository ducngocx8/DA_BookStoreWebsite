	...
	public String cart(ModelMap model, HttpSession session) {
		...
		if(cart == null) {
			...
			for(int i = 0; i<cart.size(); i++) {
				long price = cart.get(i).getProduct().getPrice();
				int idbook = cart.get(i).getProduct().getId();
				int quanty = cart.get(i).getQuanty();
				int current_price = homeServiceImpl.getBookbyID(idbook).getPrice();
				if(current_price != price) {
					cart.get(i).getProduct().setPrice(current_price);
					long total_price = quanty * current_price;
					cart.get(i).setTotal_price(total_price);
				}
			}
		}
	...
	public String checkout(HttpSession session, @RequestParam("fullname") String fullname,
		...
		if(message.length() != 0) { // TỒN KHO KHÔNG ĐỦ
			...
			if(result > 0) {
				...
				System.out.println("UPDATE TON KHO BAT DAU");
				for(int i = 0; i<cart.size(); i++) {
	        		int idbook = cart.get(i).getProduct().getId();
	        		int quanty = cart.get(i).getQuanty();
	        		int current_quanty = homeServiceImpl.getBookQuantybyID(idbook);
	        		int conlai = current_quanty - quanty;
	        		String hql = "UPDATE BookEntity B SET B.total_quantity = "+conlai+" WHERE B.id_book = "+idbook;
	        		int kq = homeServiceImpl.updateBookQuanty(hql);
	        		if(kq > 0) {
	        			System.out.println("UPDATE TON KHO CUA ID BOOK "+ idbook + " THANH CONG.....");
	        		}else {
	        			System.out.println("UPDATE TON KHO CUA ID BOOK "+ idbook + " THAT BAI.....");
	        		}
	        	}
				System.out.println("UPDATE TON KHO THANH CONG");
